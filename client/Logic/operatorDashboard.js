...
Template.editAddress.events({
  'click .delete-address': function() {
    ...
    var customerId = FlowRouter.getParam('customer');
    Meteor.call('deleteAddress', customerId, title, function(error, result) {
      if (error == undefined) {
...
Template.statusCard.events({
  'click .done': function(event) {
    ...
    Materialize.toast(
      "Good Job, you've succesfully finished serving " + name,
      4000
    );
    Meteor.call(
      'sendTxt',
      phoneNumber,
      this.customerId,
      'Good stuff! Let me know if you need anything else.',
      Meteor.userId()
    );
  ...
  'click .pending': function(event) {
    ...
    Meteor.call(
      ...
      phoneNumber,
      this.customerId,
      "I've just placed you some order. I'll get back to you shortly!",
      Meteor.userId()
    );
  ...
  'click .cancel': function(event) {
    ...
    Meteor.call(
      ...
      phoneNumber,
      this.customerId,
      "I'm so sorry, unfortunately I won't be able to help you with your order. Let me know if you need anything else!",
      Meteor.userId()
    );
    Meteor.call('endPairing', Meteor.userId(), this.customerId);
    $('.tooltipped').tooltip('remove'); // Resolves a bug that exists if you click done on the last customer you have
  ...
  'click .addFavorite'() {
    ...
    if (txt == '') {
      ...
      Meteor.call('addFavorite', this.customerId, txt, function(err, resp) {
        if (err) Materialize.toast("Sorry couldn't add a favorite", 4000);
        else {
...
Template.editFavorite.events({
  ...
  'click .delete-favorite'() {
    ...
    var customerId = FlowRouter.getParam('customer');
    Meteor.call('deleteFavorite', customerId, key, function(error, result) {
      console.log(error);
      console.log(result);
      if (error == undefined) {
        $('#editFavorite').modal('close');
  ...
  'click .addAddress'() {
    ...
    if (unique == undefined) {
      if (!title || !line1 || !city || !prov) {
        Materialize.toast(
          'Please fill in title, line1, city, province, and zip!',
          4000
        );
      } else {
        Meteor.call(
...
Template.userStatus.helpers({
  ...
  servingColor(customerId) {
    // Highlights the customer who's currently being served
    var parameter = FlowRouter.getParam('customer');
    if (customerId == parameter) return 'serving';
  },
});

Template.getUser.helpers({
  searching() {
    // Checks if operator is seeking customers or not
    var doc = OperatorProfile.findOne({ userId: Meteor.userId() });

    if (doc != undefined) var state = doc.seeking;
    else var state = false;

    return state;
  },
  customersWaiting() {
    // checks if there are customers waiting to be paired with operators = pulse to the add button
...
Template.userInfoCard.helpers({
  ...
  addresses(customerId) {
    var userAddresses = Addresses.findOne({
      userId: customerId,
    }).address.reverse();
...
Template.OperatorDashboardLayout.helpers({
  isOperator() {
    ...
    else FlowRouter.go('/'); // redirect, not an operator
  },
});
