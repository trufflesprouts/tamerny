  ...
  'submit form': function(event) {
    ...
    if (passwordVar != '') {
      ...
      if (validPhone(numberVar)) {
        // phone is valid
        Meteor.call('isEmailValid', emailVar, function(error, result) {
          var emailValid = true;
          ...
          if (emailValid == true) {
            ...
            Meteor.call('newUser', emailVar, passwordVar, function(
              err,
              response
            ) {
              if (
                response.error != undefined // meteor user not created
              )
                Materialize.toast(response.reason, 1000);
              else {
                var doc = Meteor.call('getUserIdWithEmail', emailVar, function(
                  error,
                  response
                ) {
                  if (error == undefined) {
                    ...
                    UserProfiles.insert(newUserProfile, function(err, result) {
                      if (err) {
                        ...
                        Meteor.loginWithPassword(
                          ...
                          passwordVar,
                          function(err) {
                            if (err) Materialize.toast(err, 1000);
                            else {
                              $('#login').modal('close');
                              Meteor.call(
                              ...
                              Meteor.call(
                                ...
                                number,
                                userId,
                                "Amazing! We've just successfully setup your account. I'm going to be your personal assistant fron now on. You can ask me to order you things, reserve stuff for you, deliver things for you and much more. Can I help you with anything?",
                                Meteor.userId()
                              );
                              FlowRouter.go('/profile');
                            }
    ...
    } else Materialize.toast("Password can't be empty", 3000);
  },
});
